...
function getMascotMood(w: number): { emoji: string; msg: string } {
  if (w <= 2) return { emoji: '😰', msg: 'Rozumiem, że dziś ciężko. Dobierzemy menu, które będzie łagodne dla żołądka.' };
  if (w <= 4) return { emoji: '😟', msg: 'Kiepski dzień? Skupimy się na lekkostrawnych, chłodnych posiłkach.' };
  if (w <= 6) return { emoji: '😐', msg: 'Trzymasz się! Dobrze dobrane posiłki pomogą utrzymać energię.' };
  if (w <= 8) return { emoji: '🙂', msg: 'Nieźle! Przy lepszym samopoczuciu możemy zaproponować bogatsze opcje.' };
  return { emoji: '😊', msg: 'Super dzień! Twój organizm jest gotowy na pełnowartościowe odżywienie.' };
}

export default function WelcomeScreen({ navigate, wellbeing, setWellbeing, symptoms, setSymptoms, patientName }: Props) {
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '32px 20px 24px' }}>
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <div className="rodo-note">
            ...
            <span>Twoje dane przetwarzane są lokalnie. Nie trafiają do chmury.</span>
          </div>
        </div>
      </div>
    </div>
