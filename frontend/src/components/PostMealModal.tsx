      ...
      <div className="modal-sheet">
        ...
        <div className="modal-header">
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 2, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {mealName}
            </div>
          </div>
            ...
            {selectedKeys.map(key => {
              ...
              return (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 11, color: 'var(--text)', width: 88, flexShrink: 0 }}>
                    {sym.label}
                  </span>
                    ...
                    {SEV_OPTS.map(opt => (
                      <button
                        ...
                        style={{
                          ...
                          background: selected[key] === opt.value ? opt.color : 'var(--card)',
                          color: selected[key] === opt.value ? '#fff' : 'var(--text2)',
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 11, color: 'var(--text2)', fontSize: 12, fontWeight: 600,
              cursor: 'pointer',
            }}
          >
            Pomiń
          </button>
          <button className="orange-btn" style={{ flex: 2, margin: 0 }} onClick={handleSave}>
            {selectedKeys.length > 0 ? `Zapisz (${selectedKeys.length})` : 'Brak objawów ✓'}
          </button>
        </div>
      </div>
    </>
